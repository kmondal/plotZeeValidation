...
def plotDataMC(cat, data, mc, mc_top, mc_bottom, passMVAcut, equalArea, xaxis, norm, legend):
    ...
    if (options.TeV == 8):
        legends.append(ROOT.TLegend(.70,.65,.90,.85)) #.60,.60,.85,.85
    else:
...
if (options.TeV == 8):
    ...
    txt1[-1].DrawLatex(0.60, 0.93, "35.9 fb^{-1} (13 TeV)")
else:
...
if (options.TeV == 7):
    c_single.SaveAs("sigmaEoE_nvtx_7TeV.pdf")
    c_single.SaveAs("sigmaEoE_nvtx_7TeV.root")
else:
    c_single.SaveAs("sigmaEoE_syst.png")
    c_single.SaveAs("sigmaEoE_syst.pdf")
    c_single.SaveAs("sigmaEoE_syst.root")
