...
def plotDataMC(cat, data, mc, mc_top, mc_bottom, passMVAcut, equalArea, xaxis, norm, legend):
    global hist_syst, hist_syst_up, hist_syst_down, mc_temp, mc_temp2
    global graphs

    mc_temp.append(mc[cat].Clone())
    mc_temp2.append(mc_temp[-1].Clone())
    hist_syst.append(mc[cat].Clone())
    mc_temp[-1].SetFillColor(ROOT.myColorA3)
    mc_temp[-1].SetLineColor(ROOT.kBlack)
    mc_temp2[-1].SetLineColor(ROOT.kBlack) 
    #mc_temp2[-1].SetLineWidth(2)
...
if (options.TeV == 8):
    txt2.DrawLatex(0.07, 0.9, "#scale[0.6]{#times10^{3}}")
    txt1[-1].DrawLatex(0.135, 0.93, "#bf{CMS} #it{Preliminary}")
    txt1[-1].DrawLatex(0.56, 0.93, "35.9 fb^{-1} (13 TeV)")
else:
...
if (options.TeV == 7):
    ...
    c_single.SaveAs("idmva_syst_pas_v2.pdf")
    c_single.SaveAs("idmva_syst_pas_v2.png")
    c_single.SaveAs("idmva_syst_pas_v2.root")
